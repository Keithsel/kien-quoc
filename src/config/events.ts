...
/**
 * Scale project requirements based on active team count (2-6 teams)
 *
 * Scaling rules:
 * - minTotal: Scales proportionally (6 teams = 100%, 5 = 83%, 4 = 67%, 3 = 50%, 2 = 33%)
 * - minTeams: Base requirement minus (6 - activeTeams), minimum 1
 *   e.g., base minTeams=4: 6 teams→4, 5 teams→3, 4 teams→2, 3 teams→1, 2 teams→1
 *
 * @param event - The turn event to scale
 * @param activeTeams - Number of active teams (2-6)
 */
export function getScaledRequirements(event: TurnEvent, activeTeams: number): { minTotal: number; minTeams: number } {
  // Clamp to valid range (2-6 teams)
  const teams = Math.max(2, Math.min(6, activeTeams));

  // Scale RP proportionally: 6 teams = full, fewer = proportional reduction
  const scaleFactor = teams / 6;
  const scaledMinTotal = Math.ceil(event.minTotal * scaleFactor);

  // Decrease minTeams by 1 for each team lacking from 6
  // e.g., 6 teams = base, 5 teams = base-1, 4 = base-2, etc.
  const teamsLacking = 6 - teams;
