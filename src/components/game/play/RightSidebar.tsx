...
import { useLeaderboard, usePhaseChecks } from '~/lib/hooks';
...
export default function RightSidebar(props: RightSidebarProps) {
  ...
  const game = useGame();
  const { isResultPhase } = usePhaseChecks();
  const { leaderboard, underdogTeams } = useLeaderboard();
        ...
        <div class="space-y-2">
          <For each={leaderboard()}>
            {(team) => {
              ...
              const showChange = isResultPhase() && team.pointsChange > 0;
