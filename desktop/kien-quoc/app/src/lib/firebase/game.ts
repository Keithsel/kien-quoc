...
import { db, ensureAuth, getCurrentUserId } from './client';
...
export async function hostGame(password: string): Promise<boolean> {
  ...
  const existingGame = snapshot.val() as OnlineGameData | null;

  // Only reset if no game exists or game is in lobby state
  if (!existingGame || existingGame.status === 'lobby') {
    await set(gameRef, createInitialGameData(user.uid));
  }
...
export async function joinTeam(regionId: RegionId): Promise<void> {
  ...
  const game = snapshot.val() as OnlineGameData;
...
export async function reconnectTeam(): Promise<RegionId | null> {
  ...
  for (const [regionId, team] of Object.entries(game.teams) as [RegionId, OnlineTeam][]) {
    if (team.ownerId === user.uid) {
      ...
      const teamRef = ref(db!, `${GAME_PATH}/teams/${regionId}`);
      await update(teamRef, { connected: true });
...
export async function startOnlineGame(): Promise<void> {
  ...
  // Bot-only mode: spawn bots for all empty slots
  if (isBotOnlyMode()) {
    const botCount = getBotCount();
    const regionIds = REGIONS.slice(0, botCount).map((r) => r.id);
    const updates: Record<string, unknown> = {};

    for (const regionId of regionIds) {
      updates[`teams/${regionId}/isAI`] = true;
      updates[`teams/${regionId}/ownerId`] = 'bot';
      updates[`teams/${regionId}/connected`] = true;
    }

    const activeTeams = botCount + 1; // bots + the one human player

    updates['status'] = 'playing';
    updates['currentTurn'] = 1;
    updates['currentPhase'] = 'event';
    updates['phaseEndTime'] = Date.now() + 24 * 60 * 60 * 1000; // Event phase: no timer
    updates['turnActiveTeams'] = activeTeams;
    updates['currentEvent'] = createStoredEvent(1, activeTeams);
    updates['project'] = { totalRP: 0, teamCount: 0, success: null };

    await update(gameRef, updates);
    return;
  }

  // Normal mode: check team count
...
async function processOnlineResolution(): Promise<void> {
  ...
  const updates: Record<string, unknown> = {
    ...
    lastTurnResult: {
      ...
      totalRP: result.totalRP,
      teamCount: result.teamCount,
      indexChanges: { ...indexChanges, ...boosts },
      zoneBoosts: boosts,
...
// Map to store AI agents for each team
const aiAgents = new Map<RegionId, import('~/lib/ai').RealisticAdaptiveAgent>();
...
export async function runAITurns(): Promise<void> {
  ...
  // Lazy load AI module
  const { RealisticAdaptiveAgent } = await import('~/lib/ai');
...
// Clear AI agents when game resets
export function clearAIAgents(): void {
  aiAgents.clear();
}
