...
interface BoardCellProps {
  ...
  onClick: () => void;
  hostMode?: boolean; // When true, shows allocations live during action phase (for host view)
}
...
export default function BoardCell(props: BoardCellProps) {
  ...
  const cellScores = createMemo(() => {
    ...
    return calculateCellScores(props.cell.id, allPlacements as Record<RegionId, Record<string, number>>);
  });
  ...
  return (
    ...
    >
      ...
      <Show
        when={bgImage}
        fallback={<div class={`absolute inset-0 bg-gradient-to-br ${cellColors[props.cell.type]}`} />}
      >
        {/* Image background */}
      ...
      <div
        class={`absolute top-2 left-2 z-30 flex items-center gap-1 rounded-md px-1.5 py-0.5 bg-gradient-to-r ${cellColors[props.cell.type]}`}
      >
      ...
      <Show when={props.hostMode || !isRevealState()}>
        <div
          class={`relative z-20 p-2 flex flex-col h-full ${bgImage ? 'bg-gradient-to-b from-black/60 via-transparent to-black/60' : ''}`}
        >
          ...
          <div class="flex flex-wrap gap-1 mt-auto justify-end">
            <For each={props.cell.indices}>
              {(idx) => (
                <span
                  class={`text-[10px] px-1 py-0.5 rounded font-medium ${bgImage ? 'bg-black/50 backdrop-blur-sm' : 'bg-black/20'}`}
                >
                  {INDEX_LABELS[idx]}
                </span>
              )}
