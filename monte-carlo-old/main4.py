"""
KIẾN QUỐC KÝ - MONTE CARLO BALANCE TEST v4.0 (OPTIMAL)
=======================================================
Mục tiêu:
- Tỷ lệ kết thúc sớm: 5-20%
- Tỷ lệ thành công dự án: 40-70% mỗi lượt
- Cân bằng giữa các đội (p-value > 0.05)
- Chỉ số giảm dần nhưng không quá nhanh

Điều chỉnh từ v3.0:
- ENTROPY_PER_TURN: 1 → 2
- Bỏ RECOVERY mechanism
- Giảm chỉ số khởi đầu về mức trung bình
- Tăng yêu cầu dự án
- Giảm index_change từ các ô
"""
...
# ============================================================
# THÔNG SỐ CÂN BẰNG v4.0 - TỐI ƯU
# ============================================================

# Nguồn lực khởi đầu (tổng = 8, CÂN BẰNG hơn giữa các đội)
STARTING_RESOURCES = {
    Team.A: {"capital": 3, "labor": 3, "knowledge": 2},  # Thủ đô: cân bằng
    Team.B: {"capital": 3, "labor": 3, "knowledge": 2},  # Duyên hải: như nhau
    Team.C: {"capital": 3, "labor": 3, "knowledge": 2},  # Cao nguyên: như nhau
    Team.D: {"capital": 3, "labor": 3, "knowledge": 2},  # Đồng bằng: như nhau
    Team.E: {"capital": 3, "labor": 3, "knowledge": 2},  # Đô thị mới: như nhau
}

RESOURCES_PER_TURN = 6

# === v4.0: Chỉ số khởi đầu TRUNG BÌNH ===
STARTING_INDICES = {
    "economy": 18,
    "society": 18,
    "culture": 15,
...
# === v4.0: Entropy TỐI ƯU ===
ENTROPY_PER_TURN = 2  # Quay lại mức v2.0

# === v4.0: BỎ Recovery mechanism ===
...
# === v4.0: Giảm index_change từ các ô ===
CELL_INDEX_MAPPING = {
    "Cửa khẩu Bắc": ["integration"],
    "Khu CN Biên giới": ["economy"],
    "Vùng Văn hóa": ["culture"],
    "Cảng biển": ["economy"],
    "Nông thôn": ["society"],
    "Khu Đô thị": ["economy"],
    "Vùng Sinh thái": ["environment"],
    "Trung tâm KH": ["science"],
    "Đồng bằng": ["society"],
    "Khu Giáo dục": ["science"],
    "Hub Công nghệ": ["science"],
    "Cảng Quốc tế": ["integration"],
    "Dự án 1": ["economy"],
    "Dự án 2": ["culture"],
    "Dự án 3": ["environment"],
    "Dự án 4": ["science"],
...
# === v4.0: Yêu cầu dự án TỐI ƯU ===
PROJECT_REQUIREMENTS = {
    1: {"min_total": 6, "min_teams": 2, "special": None},
    2: {"min_total": 8, "min_teams": 2, "special": "need_knowledge"},
    3: {"min_total": 10, "min_teams": 3, "special": None},
    4: {"min_total": 12, "min_teams": 3, "special": None},
    5: {"min_total": 14, "min_teams": 3, "special": "need_all_types"},
    6: {"min_total": 16, "min_teams": 4, "special": None},
}

# === v4.0: Phần thưởng dự án CÂN BẰNG ===
PROJECT_REWARDS = {
    1: {"points_per_team": 6, "index_change": {"economy": 2, "society": 1}},
    2: {"points_per_team": 8, "index_change": {"culture": 2, "science": 1}},
    3: {"points_per_team": 10, "index_change": {"integration": 2, "environment": 2}},
    4: {"points_per_team": 12, "index_change": {"science": 3, "economy": 1}},
    5: {"points_per_team": 14, "index_change": {"society": 2, "culture": 2, "science": 2}},
    6: {
        "points_per_team": 18,
        "index_change": {"economy": 2, "society": 2, "culture": 2, "integration": 2, "environment": 2, "science": 2},
    },
}

# === v4.0: Phạt dự án TRUNG BÌNH ===
PROJECT_PENALTIES = {
    1: {"index_change": {"economy": -2}, "final_multiplier": 1.0},
    2: {"index_change": {"culture": -2}, "final_multiplier": 1.0},
    3: {"index_change": {"environment": -2}, "final_multiplier": 0.95},
    4: {"index_change": {"science": -2}, "final_multiplier": 0.95},
    5: {"index_change": {"society": -2, "culture": -1}, "final_multiplier": 0.9},
    6: {"index_change": {"economy": -2, "society": -2}, "final_multiplier": 0.85},
}

# === v4.0: Cell multiplier CÂN BẰNG ===
...
# === v4.0: Index change divisor (giảm buff từ ô) ===
INDEX_CHANGE_DIVISOR = 8  # Thay vì 5 như trước
...
# ============================================================
# AGENTS (AI STRATEGIES) - v4.0 CÂN BẰNG
# ============================================================
...
class AggressiveAgent(BaseAgent):
    ...
    def decide_placements(self, resources: Resources, game_state: "GameState") -> List[CellPlacement]:
        ...
        # === v4.0: Chia nguồn lực cho nhiều ô hơn ===
        if competitive_cells:
            # Chọn 2 ô cạnh tranh
            targets = random.sample(competitive_cells, min(2, len(competitive_cells)))
            total = sum(available.values())
            per_target = total // len(targets)

            for i, target in enumerate(targets):
                # Phân chia nguồn lực
                if i == 0:
                    res = Resources(
                        capital=available["capital"] // 2 + available["capital"] % 2,
                        labor=available["labor"] // 2 + available["labor"] % 2,
                        knowledge=available["knowledge"] // 2 + available["knowledge"] % 2,
                    )
                else:
                    res = Resources(
                        capital=available["capital"] // 2,
                        labor=available["labor"] // 2,
                        knowledge=available["knowledge"] // 2,
                    )
                if res.total() > 0:
...
class CooperativeAgent(BaseAgent):
    ...
    def decide_placements(self, resources: Resources, game_state: "GameState") -> List[CellPlacement]:
        ...
        # Ưu tiên ô synergy, cooperation, và project
        preferred_cells = [
            pos
...
class BalancedAgent(BaseAgent):
    ...
    def decide_placements(self, resources: Resources, game_state: "GameState") -> List[CellPlacement]:
        ...
        # Tìm chỉ số thấp nhất
        indices = game_state.national_indices
        sorted_indices = sorted(indices.items(), key=lambda x: x[1])
        low_indices = [k for k, v in sorted_indices[:2]]  # 2 chỉ số thấp nhất

        # Tìm ô ảnh hưởng các chỉ số thấp
        priority_cells = []
        for pos, info in BOARD_CELLS.items():
            cell_name = info["name"]
        ...
        # Loại bỏ trùng lặp và chọn
        priority_cells = list(set(priority_cells))
        cells_to_use = random.sample(priority_cells, min(3, len(priority_cells)))
...
class AdaptiveAgent(BaseAgent):
    ...
    def update_strategy(self, game_state: "GameState"):
        ...
        danger_indices = [k for k, v in game_state.national_indices.items() if v < 8]
...
class ProjectFocusedAgent(BaseAgent):
    ...
    def decide_placements(self, resources: Resources, game_state: "GameState") -> List[CellPlacement]:
        ...
        # === v4.0: Giảm tỷ lệ project xuống 50% ===
        total = sum(available.values())
        project_amount = int(total * 0.5)
        ...
        if other_cells and sum(available.values()) > 0:
            targets = random.sample(other_cells, min(2, len(other_cells)))
            remaining_total = sum(available.values())
            per_target = remaining_total // len(targets)
            ...
            for i, target in enumerate(targets):
                if i == len(targets) - 1:
                    ...
                    other_res = Resources(
                        capital=min(available["capital"], per_target // 3 + 1),
                        labor=min(available["labor"], per_target // 3 + 1),
                        knowledge=min(available["knowledge"], per_target // 3 + 1),
                    )
                    available["capital"] -= other_res.capital
                    available["labor"] -= other_res.labor
                    available["knowledge"] -= other_res.knowledge

                if other_res.total() > 0:
                    placements.append(CellPlacement(target, other_res))
...
class GameSimulator:
    ...
    def simulate_turn(self) -> TurnResult:
        ...
        # Phase 2: Quyết định đặt nguồn lực
        ...
        # Phase 6: Cập nhật điểm và chỉ số
        self._update_scores_and_indices(cell_results, project_result)

        # Phase 7: Áp dụng entropy
        self._apply_entropy()

        # Phase 8: Reset nguồn lực
        for team in Team:
...
class BalanceAnalyzer:
    ...
    def generate_report(self) -> str:
        lines = []
        lines.append("=" * 60)
        lines.append("BÁO CÁO CÂN BẰNG GAME - KIẾN QUỐC KÝ v4.0")
        lines.append("=" * 60)
...
def plot_results(metrics: SimulationMetrics, save_path: Optional[str] = None):
    ...
    fig.suptitle("KIẾN QUỐC KÝ - Balance Analysis v4.0", fontsize=14, fontweight="bold")
...
def run_monte_carlo(num_games: int = 5000, agent_config: str = "mixed", seed: int = 42) -> Dict:
    ...
    print(f"\nKIẾN QUỐC KÝ - MONTE CARLO BALANCE TEST v4.0")
    print(f"=" * 50)
...
def run_full_analysis(num_games: int = 5000, seed: int = 42):
    ...
    print("\n" + "=" * 60)
    print("KIẾN QUỐC KÝ - MONTE CARLO SIMULATION v4.0")
    print("=" * 60)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--full":
            ...
            print("\nĐang tạo biểu đồ...")
            plot_results(result["metrics"], save_path="balance_report_v4.png")
    else:
        result = run_monte_carlo(NUM_GAMES, CONFIG, SEED)
        print("\nĐang tạo biểu đồ...")
        plot_results(result["metrics"], save_path="balance_report_v4.png")
