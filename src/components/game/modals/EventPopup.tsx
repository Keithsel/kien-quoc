...
import { CircleCheckBig, CircleX, Target, TrendingUp, TrendingDown } from 'lucide-solid';
      ...
      <div class="bg-white rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden animate-scale-in flex flex-col max-h-[90vh]">
        ...
        <div class="p-6 overflow-y-auto custom-scrollbar">
          ...
          <Show when={props.lastTurnResult && props.turn > 1}>
            <div class="mb-5 bg-slate-100 rounded-xl p-4">
              <div class="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Báo cáo lượt trước</div>

              {/* Grid layout matching project rewards pattern */}
              <div
                class={`grid gap-2 mb-2 ${Object.keys(props.lastTurnResult!.zoneBoosts || {}).length > 0 ? 'grid-cols-2' : 'grid-cols-1'}`}
              >
                {/* Project Result Card */}
                <div class={`rounded-lg p-2.5 ${props.lastTurnResult!.success ? 'bg-green-50' : 'bg-red-50'}`}>
                  <div
                    class={`text-xs font-semibold mb-1 flex items-center gap-1 ${props.lastTurnResult!.success ? 'text-green-700' : 'text-red-700'}`}
                  >
                    {props.lastTurnResult!.success ? <CircleCheckBig class="w-3 h-3" /> : <CircleX class="w-3 h-3" />}
                    Dự án {props.lastTurnResult!.success ? 'thành công' : 'thất bại'}
                  </div>
                  <Show when={Object.keys(props.lastTurnResult!.indexChanges || {}).length > 0}>
                    <div
                      class={`text-xs space-y-0.5 ${props.lastTurnResult!.success ? 'text-green-600' : 'text-red-600'}`}
                    >
                ...
                {/* Zone Boosts Card - only show if there are boosts */}
                <Show when={Object.keys(props.lastTurnResult!.zoneBoosts || {}).length > 0}>
                  <div class="bg-amber-50 rounded-lg p-2.5">
                    <div class="text-xs font-semibold text-amber-700 mb-1 flex items-center gap-1">
              ...
              <div class="bg-gray-100 rounded-lg px-2.5 py-1.5 flex items-center justify-between">
                <span class="text-xs font-medium text-gray-600">Chi phí duy trì</span>
                <span class="text-xs font-semibold text-gray-500">-1 mỗi chỉ số</span>
              </div>
            </div>
          </Show>
