...
export class OnlineMode implements IGameMode {
  ...
  private async processResolution(): Promise<void> {
    ...
    for (const [key, value] of Object.entries(finalIndices)) {
      if (value <= 0) {
        const ranking = Object.entries(data.teams)
    ...
    for (const [regionId, team] of Object.entries(data.teams) as [string, FirebaseTeam][]) {
      const pointsEarned = result.teamPoints[regionId as RegionId] || 0;
    ...
    const historyEntry = {
      ...
      event: {
        ...
        project: data.currentEvent?.project || ''
      },
    ...
    updates.turnHistory = [...existingHistory, historyEntry];
  ...
  private async processEndOfTurn(): Promise<void> {
    ...
    if (data.currentTurn >= MAX_TURNS) {
      const teamEntries = Object.entries(data.teams) as [string, FirebaseTeam][];
      const ranking = teamEntries
        .filter(([, t]) => (t.ownerId && t.connected) || t.isAI)
